...
class StatusBar(object):
    szarancza = os.path.join('grafika', 'szarancza', 'szarancza_stoi.png')
    kwiat = os.path.join('grafika', 'status_flower.png')

    def __init__(self, pos, ile_kwiatow, ile_szaranczy, size=50, pionowy=True):
        self.pos = pos
        self.size = size
        self.ile_kwiatow = ile_kwiatow
        self.ile_szaranczy = ile_szaranczy
        self.pionowy = pionowy
        self.obszary_kwiatow = self.oblicz_obszary_statusu(pos, size, ile_kwiatow, pionowy)
        if pionowy:
        ...
        self.obszary_szaranczy = self.oblicz_obszary_statusu(szarancza_start_pos, size, ile_szaranczy, pionowy)
